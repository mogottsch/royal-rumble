...
class DrinkDistributionRecorder
{
    public function recordEliminationReward(
        ...
        DB::transaction(function () use ($lobby, $elimination, $offender, $victim, $giver, $splits): void {
            $elimination = Elimination::query()->lockForUpdate()->findOrFail($elimination->id);
    ...
    public function recordChestRewardDistribution(
        ...
        DB::transaction(function () use ($lobby, $chestReward, $giver, $splits): void {
    ...
    private function validate(
        ...
        $ownerParticipantId = Offender::query()
            ->where('elimination_id', $elimination->id)
            ->where('rumbler_id', $offender->id)
            ->value('participant_id');
        $giverOwnsOffender = $ownerParticipantId === null
            ? $giver->rumbler_id === $offender->id
            : (int) $ownerParticipantId === $giver->id;
        if (! $giverOwnsOffender) {
