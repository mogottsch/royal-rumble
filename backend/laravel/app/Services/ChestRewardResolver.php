...
class ChestRewardResolver
{
    ...
    public function resolve(
        ...
        return DB::transaction(function () use ($lobby, $chestReward, $chooser, $chestType, $preResolvedCard): array {
            $chestReward = ChestReward::query()->lockForUpdate()->findOrFail($chestReward->id);

            return $this->resolveWithinTransaction(
                $lobby,
                $chestReward,
                $chooser,
                $chestType,
    ...
    public function createAdminReward(
        ...
        return DB::transaction(fn (): array => $this->createAdminRewardWithinTransaction(
            $lobby,
            $chooser,
            $chestType,
            $cardKey
    ...
    private function createAdminRewardWithinTransaction(
        Lobby $lobby,
        Participant $chooser,
        string $chestType,
        string $cardKey
        ...
        return $this->resolveWithinTransaction($lobby, $chestReward, $chooser, $chestType, $scaled);
    }
    ...
    public function acknowledge(
        ...
        return DB::transaction(function () use ($lobby, $chestReward, $chooser): array {
            $chestReward = ChestReward::query()->lockForUpdate()->findOrFail($chestReward->id);

            return $this->acknowledgeWithinTransaction($lobby, $chestReward, $chooser);
        }, 3);
    }

    private function acknowledgeWithinTransaction(
        Lobby $lobby,
        ChestReward $chestReward,
        Participant $chooser
    ): array {
    ...
    public function resolveEffectChoice(
        ...
        return DB::transaction(function () use ($lobby, $chestReward, $chooser, $choiceKey): array {
            $chestReward = ChestReward::query()->lockForUpdate()->findOrFail($chestReward->id);

            return $this->resolveEffectChoiceWithinTransaction($lobby, $chestReward, $chooser, $choiceKey);
        }, 3);
    }

    private function resolveEffectChoiceWithinTransaction(
        Lobby $lobby,
        ChestReward $chestReward,
        Participant $chooser,
        string $choiceKey
    ...
    public function resolveTargetPick(
        ...
        return DB::transaction(function () use ($lobby, $chestReward, $chooser, $target): array {
            $chestReward = ChestReward::query()->lockForUpdate()->findOrFail($chestReward->id);

            return $this->resolveTargetPickWithinTransaction($lobby, $chestReward, $chooser, $target);
        }, 3);
    }

    private function resolveTargetPickWithinTransaction(
        Lobby $lobby,
        ChestReward $chestReward,
        Participant $chooser,
        Participant $target
    ...
    private function resolveDynamicCard(Lobby $lobby, Participant $chooser, array $card): array
    {
        ...
        return match ($card['key'] ?? null) {
            ...
            'safe_stable_hands', 'group_stable_hands', 'chaos_open_tab', 'chaos_blood_price' => array_merge($card, [
                'schluecke' => ($card['key'] ?? null) === 'chaos_blood_price'
                    ? 0
    ...
    private function applyAutoEffect(
        ...
        if (($card['self_shots'] ?? 0) > 0 || ($card['self_schluecke'] ?? 0) > 0) {
            $affectedParticipantIds->push($chooser->id);
        }
